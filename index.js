...
console.log({currentBr});
  ...
  cron.schedule("*/15 * * * *", async () => {
    ...
    console.log({ LINKS: links[0] || "EMPTY" });
    /// if links object is empty
    if (links.length < 1 || links.length == 1 ) {
      return;
    }  

    if(links[0].id === 'urban_dict'){
target = 1;
    }
    if (links[target].lang === "zulu") {
      await nextTabZulu(browser, links[target].link);
    } else {
      await nextTab(browser, links[target].link);
    }
  });
